                            ...
                            if (in_array($row->sub_type, ['fund_transfer', 'deposit', 'payable', 'stock', 'opening_balance'])) {
                                ...
                                if (!empty($row->transaction->type)) {
                                    if ($row->transaction->type == 'purchase') {
                                        ...
                                        if ($row->transaction->contact->type == 'customer') {
                                            $details = '<b>' . 'Security Deposit þýþýCustomer ' . '</b>' . $row->transaction->contact->name . '<br><b> Payment Ref No.</b> ' . $row->transaction->ref_no;
                                        }
                                        if ($row->transaction->contact->type == 'supplier') {
                                            $details = '<b>' . 'Security Deposit þýþýSupplier ' . '</b>' . $row->transaction->contact->name . '<br><b> Payment Ref No.</b> ' . $row->transaction->ref_no;
                                        }
                                    } elseif ($row->transaction->type == 'refund_security_deposit') {
                                        if ($row->transaction->contact->type == 'customer') {
                                            $details = '<b>' . 'Refund Security Deposit þýþýCustomer ' . '</b>' . $row->transaction->contact->name . '<br><b> Payment Ref No.</b> ' . $row->transaction->ref_no;
                                        }
                                        if ($row->transaction->contact->type == 'supplier') {
                                            $details = '<b>' . 'Refund Security Deposit þýþýSupplier ' . '</b>' . $row->transaction->contact->name . '<br><b> Payment Ref No.</b> ' . $row->transaction->ref_no;
                                        }
