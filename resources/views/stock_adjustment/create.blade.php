...
<style>
    ...
    #select2-inventory_adjustment_account-results li[aria-disabled='true'] {
        display: none;
    }
</style>
			...
			<div class="row">
				...
				<div class="col-sm-4">
					<div class="form-group">
                        ...
                        <select name="inventory_adjustment_account" id="inventory_adjustment_account"
                                placeholder="__('messages.please_select')" class='form-control select2' required>
                            @foreach($inventory_adjustment_accounts as $in_act)
                                @if($in_act->id)
                                <option value="{{ $in_act->id }}"
                                        data-def-act-type="{{ $in_act->account_type->default_account_type_id }}">
                                    {{ $in_act->name }}</option>
                                @endif
                            @endforeach
                        </select>
...
@section('javascript')
	...
	<script>
        ...
        function toggleIncomeExpenseActs(evt){
            let product_row_type = $(this).find(":selected").val();

            $(' #inventory_adjustment_account option').each(function(){
                let defAcType = $(this).data("defActType");

                if($(this).data("option-hide") == true){
                    $(this).data("option-hide", false);
                    $(this).prop('disabled', false);
                }
                if ($(this).data("defActType") != product_row_type) {
                    $(this).prop('disabled', true);
                    $(this).data("option-hide", true);
                }
            });

            $(' #inventory_adjustment_account').val(
                $(' #inventory_adjustment_account option[disabled!="disabled"]').first().val()
            ).trigger("change");
        }
        $(document).on('change', '.product_row .inventory_type_id', toggleIncomeExpenseActs);
	</script>
