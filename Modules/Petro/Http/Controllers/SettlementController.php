...
class SettlementController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            if (request()->ajax()) {
                ...
                $settlements = Datatables::of($query)
                    ->addColumn(
                        'action',
                        function ($row) use ($first,$delete_settlement) {
                            $html = '';
                            if ($row->status == 1) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            foreach ($settlement->cash_payments as $cash_payment) {
                ...
                $type = 'credit';
                $this->createAccountTransaction($transaction, $type, $account_id, $cash_transaction_payment->id, 'null', $cash_payment->customer_id,  $settlement->cash_payments->sum('amount'));
    ...
    public function saveCustomerPayment(Request $request)
    {
        try {
            ...
            // $ob_transaction_data = [
            //     'amount' => $request->amount,
            //     'account_id' => Account::where('business_id', $business_id)->where('name', 'Account Receivable ')->first()->id,
            //     'type' => 'credit',
            //     'sub_type' => 'deposit',
    ...
    public function getProductsByStoreId(Request $request)
    {
        ...
        $tab = $request->tab?? 0;
        if($store_id){
            return $this->transactionUtil->getProductsByStoreId($business_id, $location_id, $store_id,$tab);
        }else{
            $products = [];
